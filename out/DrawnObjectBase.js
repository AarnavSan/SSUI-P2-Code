...
export class DrawnObjectBase {
    ...
    set w(v) {
        //=== YOUR CODE HERE ===
        //Set the width of the object to the new value but fixed within the width configuration limit
    ...
    set wConfig(v) {
        //=== YOUR CODE HERE ===
        //Check if the new configuration is different from the current configuration
        if (!SizeConfig.eq(v, this.wConfig)) {
            this._wConfig = v;
            this.damageAll();
        }
    }
    ...
    set h(v) {
        //=== YOUR CODE HERE ===
        //Set the height of the object to the new value but fixed within the height configuration limit
    ...
    set hConfig(v) {
        //=== YOUR CODE HERE ===
        //Check if the new configuration is different from the current configuration
        if (!SizeConfig.eq(v, this.hConfig)) {
            this._hConfig = v;
            this.damageAll();
        }
    }
    ...
    applyClip(ctx, clipx, clipy, clipw, cliph) {
        ...
        //Begin a new path
        ctx.beginPath();
        //Create a rectangle path based on the given parameters
        ctx.rect(clipx, clipy, clipw, cliph);
        //Clip the context to the path
        ctx.clip();
    }
    ...
    _startChildDraw(childIndx, ctx) {
        ...
        ctx.save();
        //=== YOUR CODE HERE ===
        // move to the child's coordinate system
        ctx.translate(this.children[childIndx].x, this.children[childIndx].y);
        // reduce the clipping region of the context object so it does not include any area outside the child's bounding box
        this.makeBoundingBoxPath(ctx); // draw the bounding box of the child
        // clip the context to the path
        this.applyClip(ctx, 0, 0, Math.min(this.children[childIndx].w, this.w), Math.min(this.children[childIndx].h, this.h));
    }
    //. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
    ...
    damageArea(xv, yv, wv, hv) {
        //=== YOUR CODE HERE ===
        // Ensure the area is within the bounds of this object
        let maxX = this._x + this._w; // Max X boundary of this object
        let maxY = this._y + this._h; // Max Y boundary of this object
        // Calculate the damaged X and Y
        let damagedX = Math.max(this._x, xv);
        let damagedY = Math.max(this._y, yv);
        // Calculate the damaged width and height
        let damagedW = Math.min(wv, maxX - damagedX);
        let damagedH = Math.min(hv, maxY - damagedY);
        // If there's any valid damaged area
        if (damagedW > 0 && damagedH > 0) {
            // Report this damage up to the parent
            if (this._parent) {
                this._parent._damageFromChild(this, damagedX, damagedY, damagedW, damagedH);
            }
        }
    }
    //. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
    ...
    _damageFromChild(child, xInChildCoords, yInChildCoords, wv, hv) {
        //=== YOUR CODE HERE ===
        // Convert child's coordinates to this object's coordinates
        let xInParentCoords = child._x + xInChildCoords;
        let yInParentCoords = child._y + yInChildCoords;
        // Damage the area in this object's bounds
