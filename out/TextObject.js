...
export class TextObject extends DrawnObjectBase {
    ...
    set text(v) {
        ...
        if (this._text !== v) {
            this._text = v;
            this._recalcSize(this._findDrawContext()); //recalculate the size of the text
        }
    }
    ...
    _recalcSize(ctx) {
        //=== YOUR CODE HERE ===
        // measure our text
        const meas = this._measureText(this.text, this.font, ctx ? ctx : this._findDrawContext()); //measure the text
        // set our size to be the size of the text plus padding
        this.w = meas.w + this.padding.w;
        this.h = meas.h + this.padding.h;
    ...
    _drawSelfOnly(ctx) {
        ...
        try {
            ...
            //=== YOUR CODE HERE ===
            //Set the fillStyle and strokeStyle to the color
            ctx.fillStyle = clr;
            ctx.strokeStyle = clr;
            //Set the font
            ctx.font = this.font;
            ...
            // draw the text based on the render type
            if (this.renderType === 'fill') {
                ctx.fillText(this.text, 0, meas.baseln);
            }
            else {
                ctx.strokeText(this.text, 0, meas.baseln);
            }
        }
        finally {
