...
export class Row extends Group { 
    ...
    protected override _doLocalSizing() : void {
        ...
        this.children.forEach(element => {
            ...
            minSum += element.minW;
            natSum += element.naturalW;
            maxSum += element.maxW;

            //Set the width configurations
            hMin = Math.max(hMin, element.minH);
            hNat = Math.max(hNat, element.naturalH);
            hMax = Math.max(hMax, element.maxH);
            
        });

        //Set the height configurations
        this._wConfig = new SizeConfig(minSum, natSum, maxSum);

        //Set the width configurations
        this._hConfig = new SizeConfig(hMin, hNat, hMax);
    }
    ...
    protected _measureChildren() : [number, number, number] {
        ...
        this.children.forEach(element => {
            //Check if the child is a spring
            console.log(element.debugID);
            
            if(element instanceof Spring)
            {
                //Increment the number of springs
                numSprings++;
            }
            else
            {
                //Increment the natural size of the non-spring children
                natSum += element.naturalW;

                //Increment the total compression available across the non-spring children
                availCompr = element.naturalW - element.minW;
                console.log(element.naturalW, element.minW);
            }
        });
        console.log(natSum, availCompr, numSprings);
    ...
    protected _expandChildSprings(excess : number, numSprings : number) : void {
        //=== YOUR CODE HERE ===
        //Check if there are any springs
        if(numSprings === 0) return;

        //Calculate the amount of space each spring should expand by
        let spacePerSpring = excess / numSprings;

        //Expand each spring by the calculated space
        this.children.forEach(element => {
            if(element instanceof Spring)
            {
                element.w = spacePerSpring;
            }
        });
    }
        ...
        availCompr : number)  // total compressability across all children
    {
        ...
        for (let child of this.children) {
            ...
            if (!(child instanceof Spring)) {
                ...
                //Calculate the amount of the shortfall that this child should cover
                let assignedWidth = fraction * shortfall;

                //Set the height of the child to the assigned height
                child.w = child.wConfig.nat - assignedWidth;
            }
        }
}
    ...
    protected override _completeLocalLayout() : void { 
        ...
        switch(this._hJustification)
        {
            ...
            case 'center':
                this.children.forEach(element => {
                    element.y = (this.h - element.h) / 2;
                });
                break;
            case 'bottom':
                this.children.forEach(element => {
                    element.y = this.h - element.h;
                });
                break;
        }
    }
