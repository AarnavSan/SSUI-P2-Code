...
export class TopObject extends DrawnObjectBase {
    ...
    protected override _drawSelfOnly(ctx: CanvasRenderingContext2D): void {
        //=== YOUR CODE HERE ===
        ctx.clearRect(0, 0, this.w, this.h); //Clear the canvas behind the children
    }
    ...
    public layoutAndDrawAll() : void {
        ...
        if (this.damaged) {
            ...
            try {
                ...
                //Clip to our bounds
                this.canvasContext.rect(0, 0, this.w, this.h);
    ...
    public override damageArea(xv: number, yv: number, wv: number, hv: number): void {
        ...
        //Use the min and max functions to clamp the damage rectangle to the bounds of the canvas
        this._damageRectX = Math.min(this._damageRectX, xv);
        this._damageRectY = Math.min(this._damageRectY, yv);
        this._damageRectW = Math.max(this._damageRectW, xv + wv);
        this._damageRectH = Math.max(this._damageRectH, yv + hv);

        // record that we are damaged
        this._damaged = true;
    }
