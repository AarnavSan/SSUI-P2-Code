...
export class Column extends Group { 
    ...
    protected override _doLocalSizing() : void {
        //=== YOUR CODE HERE ===
        //Declare variables to calculate the configurations
        let minSum = 0;
        let natSum = 0;
        let maxSum = 0;
        let wMin = 0;
        ...
        this.children.forEach(element => {
           //Calculate the height configurations
            minSum += element.hConfig.min;
            natSum += element.hConfig.nat;
            maxSum += element.hConfig.max;

            //Calculate the width configurations
            wMin = Math.max(wMin, element.wConfig.min);
            wNat = Math.max(wNat, element.wConfig.nat);
            wMax = Math.max(wMax, element.wConfig.max);
            
        });

        //Set the height configurations
        this._hConfig = new SizeConfig(minSum, natSum, maxSum);

        //Set the width configurations
        this._wConfig = new SizeConfig(wMin, wNat, wMax);
    }
    ...
    protected _measureChildren() : [number, number, number] {
        ...
        //=== YOUR CODE HERE ===
        //Walk across the children
        this.children.forEach(element => {
            //If the element is a spring, increment the number of springs
            if (element instanceof Spring) {
                numSprings++;
            } 
            //If the element is not a spring, add the natural size to the natural sum
            //and calclulate the available compression
            else {
                natSum += element.hConfig.nat;
                availCompr += element.hConfig.nat - element.hConfig.min;
            }
        });
    ...
    protected _expandChildSprings(excess : number, numSprings : number) : void {
        ...
        //Calculate the amount of space each spring should take up
        let springSpace = excess / numSprings;

        //Iterate through all the children
        this.children.forEach(element => {
            if(element instanceof Spring) {
                //Set the height of the spring to the space it should take up
                element.h = springSpace;
            }
        });
    }
        ...
        availCompr : number)  // total compressability across all children
    {
        ...
        for (let child of this.children) {
            ...
            if (!(child instanceof Spring)) {
                // Calculate the compressibility of this child
                const childCompr = child.hConfig.nat - child.hConfig.min;
    
                // Avoid division by zero in case of no compressibility
                if (childCompr <= 0) continue;
    
                // Calculate the fraction of total compressibility this child represents
                const fraction = childCompr / availCompr;
    
                // Calculate the amount by which this child should be compressed
                const compressAmount = fraction * shortfall;
    
                // Set the height of the child, ensuring it doesn't go below its minimum
                child.h = Math.max(child.hConfig.nat - compressAmount, child.hConfig.min);
            }    
        }
}
    ...
    protected override _completeLocalLayout() : void { 
        ...
        //=== YOUR CODE HERE ===
        //Use Switch statement to determine the justification
        switch (this.wJustification) {
            case 'center':
                //Iterate through all the children
                this.children.forEach(element => {
                    //Set the x position of the child to the center
            ...
            case 'right':
                //Iterate through all the children
                this.children.forEach(element => {
                    //Set the x position of the child to the right
                    element.x = this.w - element.w;
                });
                break;
            ...
            case 'left':
                //Iterate through all the children
                this.children.forEach(element => {
                    //Set the x position of the child to the left
                    element.x = 0;
                });
                break;
        }
